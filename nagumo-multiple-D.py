...
if __name__ == "__main__":
    ...
    for x in range(cx_l, cx_h, 1):
        for y in range(cy_l, cy_h, 1):
            u0[y,x] = Thot
    all_time_series_lowD = []
    for m in range(nsteps):
        if m % 1000 == 0:
            if m % 10000 == 0:
                print(m)
            u1d = u.flatten()   # serialize two dimensional data to one dimension
            all_time_series_lowD.append(u1d)
        D = 20
        alpha = 0.08
        u0, u = do_timestep(u0, u, D, alpha)

    all_time_series_lowD = np.array(all_time_series_lowD)
    all_time_series_lowD = all_time_series_lowD.transpose()
    ...
    for x in range(cx_l, cx_h, 1):
        for y in range(cy_l, cy_h, 1):
            u0[y,x] = Thot
    all_time_series_highD = []
    for m in range(nsteps):
        if m % 1000 == 0:
            if m % 10000 == 0:
                print(m)
            u1d = u.flatten()   # serialize two dimensional data to one dimension
            all_time_series_highD.append(u1d)
        D = 20
        alpha = 0.06
        u0, u = do_timestep(u0, u, D, alpha)

    all_time_series_highD = np.array(all_time_series_highD)
    all_time_series_highD = all_time_series_highD.transpose()

    # substitute
    index = int(a_y1 * w + a_x1)
    all_time_series[index: index + 3, :] = all_time_series_lowD[index: index + 3, :]
    index = int(a_y2 * w + a_x2)
    all_time_series[index: index + 3, :] = all_time_series_lowD[index: index + 3, :]

    index = int(c_y1 * w + c_x1)
    all_time_series[index: index + 3, :] = all_time_series_highD[index: index + 3, :]
    index = int(c_y2 * w + c_x2)
    all_time_series[index: index + 3, :] = all_time_series_highD[index: index + 3, :]
